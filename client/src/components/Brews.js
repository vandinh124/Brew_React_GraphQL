...
export default class Brews extends Component {
	...
	deleteItemFromCart = (itemToDeleteId) => {
		const filterdItems = this.state.cartItems.filter((item) => item._id !== itemToDeleteId);
		this.setState({ cartItems: filterdItems }, () => setCart(filterdItems));
	};
	render() {
		...
		return (
			...
			>
				...
				<Box display='flex' direction='column' alignItems='center'>
					...
					>
						{brews.map((brew) => (
							<Box paddingY={4} width={210} key={brew._id}>
								<Card
									image={
										<Box margin={2} height={250} width={200}>
											<Image
												fit='cover'
												alt='Brand'
												naturalHeight={1}
